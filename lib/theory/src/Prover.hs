...
closeDiffTheoryWithMaude sig thy0 autoSources =
  if autoSources && (containsPartialDeconstructions (cacheLeft items) || containsPartialDeconstructions (cacheRight items))
    then
      proveDiffTheory (const True) checkProof checkDiffProof
        (DiffTheory (L.get diffThyName thy0) h t ext sig (cacheLeft items') (cacheRight items') (diffCacheLeft items') (diffCacheRight items') items' (L.get diffThyOptions thy0) (_diffThyIsSapic thy0))
    else
      proveDiffTheory (const True) checkProof checkDiffProof
        (DiffTheory (L.get diffThyName thy0) h t ext sig (cacheLeft items) (cacheRight items) (diffCacheLeft items) (diffCacheRight items) items (L.get diffThyOptions thy0) (_diffThyIsSapic thy0))
  ...
  where
    ...
    ext            = L.get diffThyExportTactic thy0
...
closeTheoryWithMaude sig thy0 autoSources =
  if autoSources && containsPartialDeconstructions (cache items)
    then
      ...
      $ Theory (L.get thyName thy0) h t ext sig (cache items') items' (L.get thyOptions thy0)  (L.get thyIsSapic thy0)
    else
        proveTheory (const True) checkProof
      $ Theory (L.get thyName thy0) h t ext sig (cache items) items (L.get thyOptions thy0) (L.get thyIsSapic thy0)
  where
    ...
    ext        = L.get thyExportTactic thy0
...
openTheory  (Theory n h t ext sig c items opts sapic) = openTranslatedTheory(
    Theory n h t ext (toSignaturePure sig) (openRuleCache c)
...
openDiffTheory  (DiffTheory n h t ext sig c1 c2 c3 c4 items opts sapic) =
    -- We merge duplicate rules if they were split into variants
    DiffTheory n h t ext (toSignaturePure sig) (openRuleCache c1) (openRuleCache c2) (openRuleCache c3) (openRuleCache c4)
