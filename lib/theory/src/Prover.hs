  ...
  if autoSources && (containsPartialDeconstructions (cacheLeft items) || containsPartialDeconstructions (cacheRight items))
    then
      proveDiffTheory (const True) (const True) checkProof checkDiffProof
        (DiffTheory (L.get diffThyName thy0) h t sig (cacheLeft items') (cacheRight items') (diffCacheLeft items') (diffCacheRight items') items' (L.get diffThyCollapseBound thy0))
    else
      proveDiffTheory (const True) (const True) checkProof checkDiffProof
        (DiffTheory (L.get diffThyName thy0) h t sig (cacheLeft items) (cacheRight items) (diffCacheLeft items) (diffCacheRight items) items (L.get diffThyCollapseBound thy0)) 
  where
...
closeTheoryWithMaude sig thy0 autoSources =
  if autoSources && containsPartialDeconstructions (cache items)
    then
      ...
      $ Theory (L.get thyName thy0) h t sig (cache items') items' (L.get thyOptions thy0) (L.get thyCollapseBound thy0)
    else
        proveTheory (const True) checkProof
      $ Theory (L.get thyName thy0) h t sig (cache items) items (L.get thyOptions thy0) (L.get thyCollapseBound thy0)
  where
    ...
    rules its = theoryRules (Theory errClose errClose errClose errClose errClose its errClose errClose)
...
openTheory  (Theory n h t sig c items opts cb) = openTranslatedTheory(
...
openDiffTheory  (DiffTheory n h t sig c1 c2 c3 c4 items dcb) =
    -- We merge duplicate rules if they were split into variants
    DiffTheory n h t (toSignaturePure sig) (openRuleCache c1) (openRuleCache c2) (openRuleCache c3) (openRuleCache c4)
      (mergeOpenProtoRulesDiff $ map (mapDiffTheoryItem id (\(x, y) -> (x, (openProtoRule y))) (\(DiffLemma s a p) -> (DiffLemma s a (incrementalToSkeletonDiffProof p))) (\(x, Lemma a b c d e) -> (x, Lemma a b c d (incrementalToSkeletonProof e)))) items) dcb
