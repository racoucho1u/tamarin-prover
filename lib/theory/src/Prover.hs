  ...
  if autoSources && (containsPartialDeconstructions (cacheLeft items) || containsPartialDeconstructions (cacheRight items))
    then
      proveDiffTheory (const True) checkProof checkDiffProof
        (DiffTheory (L.get diffThyName thy0) (L.get diffThyInFile thy0) h t sig (cacheLeft items') (cacheRight items') (diffCacheLeft items') (diffCacheRight items') items' (L.get diffThyOptions thy0) (_diffThyIsSapic thy0) (L.get diffThyCollapseBound thy0))
    else
      proveDiffTheory (const True) checkProof checkDiffProof
        (DiffTheory (L.get diffThyName thy0) (L.get diffThyInFile thy0) h t sig (cacheLeft items) (cacheRight items) (diffCacheLeft items) (diffCacheRight items) items (L.get diffThyOptions thy0) (_diffThyIsSapic thy0) (L.get diffThyCollapseBound thy0))
...
closeTheoryWithMaude sig thy0 autoSources showSaturation =
  if autoSources && containsPartialDeconstructions (cache items)
    then
      ...
      $ Theory (L.get thyName thy0) (L.get thyInFile thy0) h t sig (cache items') items' (L.get thyOptions thy0) (L.get thyIsSapic thy0) (L.get thyCollapseBound thy0)
    else
        proveTheory (const True) checkProof
      $ Theory (L.get thyName thy0) (L.get thyInFile thy0) h t sig (cache items) items (L.get thyOptions thy0) (L.get thyIsSapic thy0) (L.get thyCollapseBound thy0)
  where
...
openTheory  (Theory n f h t sig c items opts sapic cb) = openTranslatedTheory(
...
openDiffTheory  (DiffTheory n f h t sig c1 c2 c3 c4 items opts sapic dcb) =
    -- We merge duplicate rules if they were split into variants
