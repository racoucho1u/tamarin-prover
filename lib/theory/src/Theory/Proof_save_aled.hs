  ...
  where
    ...
    prove !depth keepGoing sys =
        case rankedGoals of
          ...
          (InLoop (idx,g) , (cases, _expl)):_   -> trace ("2: InLoop fst detec "++show idx) (if keepGoing then trace ("1: keepGoing: "++ show idx ++ " "++show g) fst $ node (InLoop (idx,g)) cases False else fst $ node (InLoop (idx,g)) M.empty keepGoing )
      ...
      where
        ...
        node methodOrigin cases keepGoing =
          if (cases == M.empty) then trace ("2: endNode "++show methodOrigin) ((LNode (ProofStep methodOrigin ()) M.empty),False) else ((LNode (ProofStep method ()) successors_),skip)
        ...
        propagatedMethod cases methodOrigin keepGoing = if keepGoing then (methodOrigin, successors, False)
                ...
                InLoop (1,g)     -> trace ("2: BlackListed "++(show $ (fromJust $ extractGoal methodOrigin))) (BlackListed (fromJust $ extractGoal methodOrigin), successors, True)
                InLoop (idx,g)   -> trace ("2: InLoop "++(show $ idx-1)++" "++(show $ (fromJust $ extractGoal methodOrigin))) (InLoop (idx-1,(fromJust $ extractGoal methodOrigin)), successors, False) 
                BlackListed goal -> trace ("2: BlackListed to normal "++show methodOrigin) (methodOrigin, successors, False)
                _                -> trace ("2: Normal to normal "++show methodOrigin) (methodOrigin, successors, False)
          ...
          where
                ...
                successors = trace ("2: proving") M.map (prove (succ depth) keepGoing) cases
