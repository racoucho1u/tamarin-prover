  ...
  where
    ...
    autoProver = Prover $ \ctxt depth sys _ ->
        return $ fmap (fmap Just)
               ...
               $ proof ctxt depth sys
        where
          proof cx d s = case proveSystemDFS (selectHeuristic aut cx) (selectTactic aut cx) cx d s [] of
            LNode (ProofStep (Unfinishable []) ()) c -> LNode (ProofStep (Unfinishable []) ()) c
            LNode (ProofStep (Unfinishable sL) ()) _ -> proveSystemDFS (selectHeuristic aut cx) (selectTactic aut cx) cx d s sL
            LNode r c -> LNode r c
    ...
    prove !depth skip c sys = trace ("Prove case "++show c++": "++show (length ranked)++" | d: "++show depth) $ case ranked of --trace ("Prove case "++show c++": "++show (length ranked)++" | d: "++show depth)
      ...
      where
        ...
        propagatedMethod cases methodOrigin = case propagatingMethod skip (Sorry Nothing,"",M.empty) (M.toList cases) of
                (Incorrect (_,1, g, skl),_,_)  -> (Unfinishable skl, M.empty,skl++[cleanGoal <$> g])
        ...
        fantomNode propagMethod cases skipL = trace ("Fantom: "++prettyProofMethod propagMethod) $
          case propagMethod of
            Incorrect (_, 0, _, _) -> trace "Ending proof" node "unfinishable" [] (Unfinishable skipL) M.empty
            Incorrect (_, _, _, _) -> node "falseNode" skipL propagMethod M.empty
            m -> node "trying" skipL propagMethod cases --trace ("Not ending proof"++show depth++" | "++show (prettyProofMethod  m))-}
        ...
        prettyProofMethod method = case method of
            ...
            Incorrect (s,d,Just goal,_) -> "incorrect(" ++show goal ++ ") /*bad branch (score: "++show s++", d: "++show d++")*/"
