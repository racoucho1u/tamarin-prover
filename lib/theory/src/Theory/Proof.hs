...
proveDiffSystemDFS heuristic tactics ctxt d0 sys0 =
  ...
  where
    prove !depth ignoreGoals sys =
      ...
      where
        explore :: [(DiffProofMethod, (M.Map CaseName DiffSystem,String))] -> (DiffProofMethod, M.Map CaseName DiffSystem) -> [Maybe Goal] -> DiffProof (Maybe DiffSystem)
        explore [] (method0, cases0) igG = node method0 cases0 (fmap freeme (extractGoal method0):igG) 
        ...
        node :: DiffProofMethod -> M.Map CaseName DiffSystem -> [Maybe Goal] -> DiffProof(Maybe DiffSystem)
        node method cases igG = trace (show diffnodule)
            diffnodule 
          where
            successors = M.map (prove (succ depth) igG) cases
            diffnodule = LNode (DiffProofStep method (Just sys)) successors
