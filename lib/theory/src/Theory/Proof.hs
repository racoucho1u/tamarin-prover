...
proveSystemDFS heuristic tactics ctxt d0 sys0 =
  ...
  where
    ...
    prove !depth sys = trace ("Depth: "++show depth) $ case rankProofMethods (useHeuristic heuristic depth) tactics ctxt sys of
      ...
      where
        ...
        checkForLoop ((method, (cases, _expl)):suite) (method0, (cases0, _expl0)) = case method of 
            InLoop _ -> node method cases --checkForLoop suite (method0, (cases0, _expl0))
            _ -> node method cases

        node method cases = trace ("Node: "++show method)
