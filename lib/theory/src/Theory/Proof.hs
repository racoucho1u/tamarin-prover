...
--                      (M.singleton "" prf)
  where
    ...
    chooseNext list = case thechosenone of
      ...
      -- trace ("\nAbricot:"++show idx ++"\nAbricot:" ++ show thecho ++"\nAbricot:" ++ show (map printElem list) )
        ...
        where
            ...
            cleanElem (InLoop (s, _d, m, i),(_sys,_)) = (SolveGoal m, Just _sys)
            ...
            heuristicScore l = case scoreProba of
              [] -> 1 : replicate (ll -1) 0
              sp -> if lsp == ll then sp else sp++[last sp -1]++replicate (ll-lsp-1) 0

              where
                probaList = map computeProba l
                usableProbaList = takeWhile (<1) probaList
                ...
                computeProba (InLoop (_, _, _, it),_) = 1 / (2**(int2Double it+1))
                computeProba _ = 1.0
            ...
            iterationScore l = map computeIt itScoreList
                where
                    ...
                    retrieveIt (meth, (_,_)) = case meth of
                        InLoop (_,_,_,iteration) -> Just iteration
                        _ -> Nothing
            ...
            depthScoreMax l = map computeDepth dScoreList
                where
                    ...
                    computeDepth (Just dpth) = dpth
                    computeDepth Nothing = maxD

                    retrieveDepth (meth, (_, _)) = case meth of
                        InLoop (_,dpth,_,_) -> Just dpth
                        _ -> Nothing
            ...
            depthScoreMin l = map (maxDepth -) dList
                where
                    ...
                    dScore (meth, (_, _)) = case meth of
                        InLoop (_,dpth,_,_) -> dpth
                        _ -> 0
      ...
      where
        ...
        heuristicScore l = case scoreProba of
          ...
          where
            ...
            computeIndivProba (meth, (_,_)) = case meth of
                    DiffBackwardSearchStep (InLoop (_,_,_,iteration)) -> Just $ 1 / (2**(int2Double iteration+1))
                    _ -> Nothing
        ...
        openCasesScoreMax _list = reverse $ foldl (\acc (_, (cases, _expl)) -> length (M.toList cases):acc) [] _list

        openCasesScoreMin :: [(DiffProofMethod, (M.Map CaseName DiffSystem, String))] -> [Int]
        openCasesScoreMin _list = reverse $ map (maxLength -) casesLength
            where
                casesLength = foldl (\acc (_, (cases, _expl)) -> length (M.toList cases):acc) [] _list
                maxLength   = maximum casesLength

        iterationScore :: [(DiffProofMethod, (M.Map CaseName DiffSystem, String))] -> [Int]
        iterationScore _list = map computeIt itScoreList
            where
                itScoreList = map retrieveIt _list
                maxIt = 2 + fromMaybe 0 (maximum itScoreList)
                ...
                retrieveIt (meth, (_,_)) = case meth of
                    DiffBackwardSearchStep (InLoop (_,_,_,iteration)) -> Just iteration
                    _ -> Nothing

        depthScoreMax :: [(DiffProofMethod, (M.Map CaseName DiffSystem, String))] -> [Int]
        depthScoreMax _list = map computeDepth dScoreList
            where
                dScoreList = map retrieveDepth _list
                maxD = 2 + fromMaybe 0 (maximum dScoreList)
                ...
                retrieveDepth (meth, (_, _)) = case meth of
                    DiffBackwardSearchStep (InLoop (_,dpth,_,_)) -> Just dpth
                    _ -> Nothing

        depthScoreMin :: [(DiffProofMethod, (M.Map CaseName DiffSystem, String))] -> [Int]
        depthScoreMin _list = map (maxDepth -) dList
            where
                dList = map dScore _list
                maxDepth = maximum dList

                dScore (meth, (_, _)) = case meth of
                    DiffBackwardSearchStep (InLoop (_,dpth,_,_)) -> dpth
                    _ -> 0
...
proveSystemDFS heuristic tactics ctxt d0 sys0 = prove d0 sys0
  where
    ...
    prove !depth sys = case rankProofMethods (useHeuristic heuristic depth) tactics ctxt sys of
      ...
      where
        ...
        handler name _sys = do
            ...
            trace (render $ prettyGeneratedTactic name $ filter (\(_,it,_,_) -> it>0) (L.get sPathGoals _sys)) raiseSignal killProcess
    ...
    prove !depth sys =
        ...
        case  s (useHeuristic heuristic depth) tactics ctxt sys of
      ...
      where
        ...
        -- New version: closer to probabilistic
        heuristicScore :: [(ProofMethod, (M.Map CaseName System, String))] -> [Int]
        heuristicScore list = case scoreProba of
          [] -> 1 : replicate (ll -1) 0
          sp -> gui sp

          where
            probaList = map computeIndivProba list
            inLoopProbaList = takeWhile (<1) probaList
            inverseProba = map (1 -) inLoopProbaList
            productInverseProba = foldl (\acc x -> acc ++ [last acc * x]) [1] inverseProba
            usableProbaList = zipWith (*) productInverseProba inLoopProbaList
            ...
            computeIndivProba (InLoop (_, _, it),_) = 1 / (2**(int2Double it+1))
            computeIndivProba _ = 1.0
    ...
    prove !depth sys = case rankProofMethods (useHeuristic heuristic depth) tactics ctxt sys of
      ...
      where
        ...
        nbCases list = reverse $ foldl (\acc weight -> if acc == [] then weight:acc else (head acc+weight):acc) [] weightCases 
            where
                casesLength (_, (cases, _)) = length $ M.toList cases --tried to use size directly on M.Map but it was not recognized
                hRankedCasesNb = map casesLength list
                smallToLargeRanked = sortOn fst (zip hRankedCasesNb [0..])
                largeToSmallRanked = reverse $ sort hRankedCasesNb
                smallToLargeIdx = map snd smallToLargeRanked
                largeToSmallIdx = zip largeToSmallRanked smallToLargeIdx
    ...
    {--Avoid the loops and if no other choice, choose randomly based on the number of cases
  ...
  where
    ...
    prove !depth sys =
      ...
      where
        ...
        drawRand :: Int -> Int
        drawRand sup = unsafePerformIO $ do
            let name = "Observational_equivalence" --(L.get pcLemmaName ctxt)
            _ <- installHandler sigINT  (Catch (handler name sys)) Nothing
            _ <- installHandler sigTERM (Catch (handler name sys)) Nothing
            g <- newStdGen
            let (result, _) = randomR (0, sup) g
            return result
        ...
        incrementIteration 0 ((s,it,d,g):t) _ _ = (s,it+1,d,g):t
        incrementIteration _ [] removeint removelist = error (show removeint++" "++show (length removelist)++" "++show removelist)
        incrementIteration d (h:t) removeint removelist = h:incrementIteration (d-1) t removeint removelist
        ...
        handler name _sys = do
            ...
            trace (render $ prettyGeneratedTactic name $ filter (\(_,it,_,_) -> it>0) goalList) raiseSignal killProcess
            --putStrLn $ render $ prettyGeneratedTactic name $ L.get sPathGoals _sys
            --raiseSignal killProcess
    ...
    prove !depth sys =
        case rankDiffProofMethods (useHeuristic heuristic depth) tactics ctxt sys of
          ...
          list -> chooseNext list sys
      where
        chooseNext :: [(DiffProofMethod, (M.Map CaseName DiffSystem, String))] -> DiffSystem -> DiffProof DiffSystem
      ...
      where
        ...
        heuristicScore l = case scoreProba of
          ...
          where
            ...
            productInverseProba = foldl (\acc x -> acc ++ [last acc * x]) [1] inverseProba
            usableProbaList = zipWith (*) productInverseProba inLoopProbaList
            totalProba = product usabhandler :: String -> DiffSystem -> IO()
        handler name _sys = do
            let goalList = maybe [] (L.get sPathGoals) (L.get dsSystem _sys)
            putStrLn $ render $ prettyGeneratedTactic name $ filter (\(it,_) -> it>0) goalList
            raiseSignal killProcessleProbaList
            scoreProba = map (double2Int . (* totalProba)) usableProbaList
            lsp = length scoreProba
