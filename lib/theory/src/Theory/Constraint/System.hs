...
allOpenGoalsAreSimpleFacts ctxt sys = M.foldlWithKey goalIsSimpleFact True (L.get sGoals sys)
  where
    ...
    goalIsSimpleFact ret (SubtermG _)             (GoalStatus solved _ _ _) = ret && solved
...
instance Apply LNSubst System where
    apply subst (System a b c d e f g h i j k l m n) =
        ...
        j k (apply subst l) (apply subst m) (apply subst n)
...
compareSystemsUpToNewVars
   (System a1 b1 c1 d1 e1 f1 g1 h1 i1 j1 k1 l1 False m1)
   (System a2 b2 c2 d2 e2 f2 g2 h2 i2 j2 k2 l2 False m2)
       = if compareNodes == EQ then
            compare (System M.empty b1 c1 d1 e1 f1 g1 h1 i1 j1 k1 l1 False m1)
                (System M.empty b2 c2 d2 e2 f2 g2 h2 i2 j2 k2 l2 False m2)
         else
            compareNodes
        where
