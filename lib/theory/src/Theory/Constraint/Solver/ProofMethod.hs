...
execDiffProofMethod ctxt method sys = -- error $ show ctxt ++ show method ++ show sys -- return M.empty
  ...
  where
    ...
    isSolved s sys' = (rankProofMethods GoalNrRanking [defaultTactic] Nothing (eitherProofContext ctxt s) sys') == [] -- checks if the system is solved
...
-- system is solved.
rankProofMethods :: GoalRanking ProofContext -> [Tactic ProofContext] -> Maybe Int -> ProofContext -> System
                 -> [(ProofMethod, (M.Map CaseName System, String))]
rankProofMethods ranking tactics cbound ctxt sys =
  ...
  where
    ...
    collapse [] = DisjG (Disj {getDisj = []})
    collapse [_] =  DisjG (Disj {getDisj = []})
    collapse l = DisjG (Disj l)

    --extract timepoints from existing nodes to generate an equality distinction goal
    node2goals :: [(NodeId,NodeId)] -> [LNGuarded]
    node2goals ((nodeId1,nodeId2):t) = GAto (EqE (varTerm $ Free nodeId1) (varTerm $ Free nodeId2)) : node2goals t
    node2goals _ = []

    pairing :: [a] -> [(a,a)]
    pairing []    = []
    pairing [_]   = []
    pairing (i:l) = map (i,) l ++ pairing l
...
-- system is solved.
rankDiffProofMethods :: GoalRanking ProofContext -> [Tactic ProofContext] -> Maybe Int -> DiffProofContext -> DiffSystem
