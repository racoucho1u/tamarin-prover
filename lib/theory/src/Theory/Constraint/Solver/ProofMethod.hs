...
import           Data.Maybe                                (catMaybes, fromMaybe, mapMaybe, isNothing, isJust, listToMaybe, mapMaybe)
-- import           Data.Monoid
...
execProofMethod ctxt method sys =
    case method of
      ...
      SolveGoal goal        -> checkForLoop goal sys --return $ process $ solve goal
      InLoop (_,_, goal,_)  -> checkForLoop goal sys
      Invalidated           -> Nothing
  where
    ...
    execSolveGoal goal _loop score _index iteration = return $ process sys' $ solve goal
      where
...
execDiffProofMethod ctxt method sys =
  ...
  where
    ...
    isSolved s sys' = (isJust $ isFinished (eitherProofContext ctxt s) sys' >>= guard . (== Solved)) && not (L.get sLoopFound sys')
...
prettyGeneratedTactic s goals = kwTactic <> colon <> space <> text (s++"_generated")
   ...
   where
        ...
        ppTab xs = kwDeprio <> colon <> space $-$ nest 2 (vcat $ map (prettify . words) xs)
