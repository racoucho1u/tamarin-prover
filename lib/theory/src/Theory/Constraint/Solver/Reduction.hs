...
substPathGoals = do subst <- getM sSubst
    ...
    where
        unifyLists :: LNSubst -> [(Int,Int,Int,[Goal])] -> [(Int,Int,Int,[Goal])]
        unifyLists _ [] = []
        unifyLists subst ((score,it,depth,goal):t) = (score, it, depth, foldl  (\li x -> if x `elem` li then li else x:li) goal (apply subst goal)) : unifyLists subst t
