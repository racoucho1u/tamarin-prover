...
applyMethodAtPath thy lemmaName proofPath prover i = do
    ...
    methodsAndCases <- rankProofMethods ranking tactic cbound ctxt <$> sys
    ...
    applyProverAtPath thy lemmaName proofPath
      (oneStepProver cgoal (fst methodCases)                   `mappend`
       ...
       replaceSorryProver (oneStepProver Nothing Unfinishable) `mappend`
...
applyDiffMethodAtPath thy lemmaName proofPath prover i = do
    ...
    methods <- (map fst . rankDiffProofMethods ranking tactic cbound ctxt) <$> sys
...
subProofSnippet renderUrl renderImgUrl tidx ti lemma proofPath ctxt prf =
  ...
  where
    ...
    proofMethods            = rankProofMethods ranking tactic collapseB ctxt
...
subDiffProofSnippet renderUrl tidx ti lemma proofPath ctxt prf =
  ...
  where
    ...
    diffProofMethods        = rankDiffProofMethods ranking tactic collapseB ctxt
    subCases                = concatMap refSubCase $ M.toList $ children prf
    refSubCase (name, prf') =
        [ withTag "h4" [] (text "Case" <-> text name)
        , maybe (text "no proof state available")
      ...
      D.showDot "G" $ fromMaybe (return ()) $ do
        ...
        if mir
          then do
            ...
            let isSolved s sys' = (rankProofMethods GoalNrRanking [defaultTactic] Nothing (eitherProofContext ctxt s) sys') == [] -- checks if the system is solved
